...
boot.ci( boot(cropland_parcels_grouped_overall$cropland_area, boot_gini, R = 1000), type = "perc")
boot.ci( boot(cropland_parcels_grouped_overall$landval, boot_gini, R = 1000), type = "perc")
boot.ci( boot(cropland_parcels_grouped_overall$improvval, boot_gini, R = 1000), type = "perc")
...
boot_gini <- function(data, indices) {
  ...
  return(ineq(sample_data))}

# Run bootstrap (1000 replicates)
set.seed(123)  # For reproducibility

boot.ci( boot(final_matches_grouped_overall$normalised_depth, boot_gini, R = 1000), type = "perc")
boot.ci( boot(final_matches_grouped_overall$well_capacity_lps, boot_gini, R = 1000), type = "perc")
```
...
summary(lm(data = final_matches_grouped_overall, log(normalised_depth) ~ log(cropland_area)))
...
# Your models
model1 <- lm(data = final_matches_grouped_overall, log(normalised_depth) ~ log(cropland_area))  
model2 <- lm(data = final_matches_grouped_overall, log(well_capacity_lps) ~ log(cropland_area))


# Create a flextable
table <- msummary(
  list("Well Depth" = model1, "Well Capacity" = model2),
  stars = TRUE,
  output = "flextable")

save_as_docx(table, path = "temp_table.docx")
```
...
final_matches_grouped_overall%>%
  ...
  theme_bw(14)+
  labs(x = "Percentile of Cropland Area Owned", y = "Mean Total Normalised \n Well Depth Owned (m)")
...
final_matches_grouped_overall%>%
  ...
  geom_point(colour = "skyblue4")+
  theme_bw(14)+
  labs(x = "Percentile of Cropland Value Owned", y = "Mean Total Normalised \n Well Depth Owned (m)")
...
final_matches_grouped_overall%>%
  ...
  geom_point()+
  theme_bw(14)+
  labs(x = "Percentile of Cropland Area Owned", y = "Mean Normalised Depth per Well (m)")
...
final_matches_grouped_overall%>%
  group_by(cropland_area_percentile)%>%
  summarise(mean_depth = mean(depth_per_ha_norm, na.rm = T))%>%
  ggplot(aes(cropland_area_percentile, mean_depth))+
    geom_point(colour = "royalblue4")+
    theme_bw(14)+
    labs(x = "Percentile of Cropland Area Owned", y = "Mean Normalised Well Depth \n per Hectare (m per ha)")
    #scale_y_log10()
...
ggplot(gw_basins_matches, aes(overdrafted_label, gini_depth_norm))+
  geom_boxplot(fill = "skyblue")+
  theme_bw(15)+
  labs(x = "Subbasin Critically Overdrafted Status", y = "Gini Coefficient of \n Normalised Well Depth")
```
...
fig7 = area_ginis%>%dplyr::select(-n)%>%
  ...
  scale_x_discrete(position = "top", labels = c("gini_area" = "Cultivated Area", "gini_capacity" = "Well Capacity", "gini_depth_norm" = "Normalised Well Depth"))+
  theme_bw(25)+
...
cropland_parcels_with_agencies%>%
  mutate(cropland_area_percentile = ntile(cropland_area, 100))%>%
  group_by(cropland_area_percentile)%>%
  ...
  geom_point()+
  theme_bw(14)+
  labs(x = "Percentile of Cropland Area Owned", y = "% of Parcels Intersecting SW Districts")
...
cropland_parcels_with_agencies%>%
  mutate(sw_parcel = ifelse(agency_type %in% c("Irrigation & water districts"), 1, 0), cropland_area_percentile = ntile(cropland_area, 100))%>%
