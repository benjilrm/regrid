...
parcel_precip_means <- exact_extract(crop(precip, ext(master_cropland_parcels_covariates)), master_cropland_parcels_covariates, 'mean', force_df = TRUE)%>%
  rename_with(~ as.character(1981:2023))%>%
  mutate(mean_annual_precip = rowMeans(across(everything())))%>%
  pull(mean_annual_precip)
...
master_cropland_parcels_covariates = master_cropland_parcels_covariates%>%
...
write.csv(final_matches_joined, "Data/Well-parcel matches/final_matches_joined.csv")
```
...
# Crop analysis
```{r}
cropland_parcels_w_crop_mode = master_cropland_parcels_covariates%>%
  mutate(crop_mode = exact_extract(cropland_extent_raster, master_cropland_parcels_covariates, fun = "mode", progress = TRUE))%>%
  left_join(., data.frame(levels(cropland_extent_raster)), by = c("crop_mode" = "Value"))%>%
  filter(!grepl("Developed|Fallow|Wetlands|Pasture|Shrubland|Grass Seed|Open Water|Forest|Barren|Aquaculture", Class_Name) & !grepl(exclude_owners, owner) & !is.na(owner))%>%
  mutate(
...
st_write(cropland_parcels_w_crop_mode, "Data/Parcels/Cropland parcels/cropland_parcels_w_cropmode.gpkg", delete.dsn = T)
```


```{r}
final_matches_sf = filtered_parcels%>%
  select(my_parcel_num)%>%
  filter(my_parcel_num %in% final_matches_joined$my_parcel_num)%>%
  left_join(., final_matches_joined, by = "my_parcel_num")

st_write(final_matches_sf, "Data/Well-parcel matches/final_matches_sf.gpkg")

final_matches_w_crop_mode = final_matches_sf%>%
  mutate(crop_mode = exact_extract(cropland_extent_raster, final_matches_sf, fun = "mode", progress = TRUE))%>%
  left_join(., data.frame(levels(cropland_extent_raster)), by = c("crop_mode" = "Value"))%>%
  filter(!grepl("Developed|Fallow|Wetlands|Pasture|Shrubland|Grass Seed|Open Water|Forest|Barren|Aquaculture", Class_Name) & !grepl(exclude_owners, owner) & !is.na(owner))%>%
  mutate(
    crop_group = case_when(
...
st_write(final_matches_w_crop_mode, "Data/Well-parcel matches/final_matches_w_crop_mode.gpkg", delete.dsn = T)
```
