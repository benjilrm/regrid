...
final_matches_joined = final_matches%>%
  left_join(., filtered_wells%>%data.frame()%>%
              ...
              select(ca_well_id, well_depth_m, well_capacity_lps, well_year,
...
st_write(filtered_parcels_with_covariates, "Data/Parcels/Filtered parcels/filtered_parcels_with_covariates.gpkg")
...
unique_owners <- unique(filtered_parcels_filtered_owners$cleaned_owner) # Get unique cleaned owner names
dist_matrix <- stringdist::stringdistmatrix(unique_owners, unique_owners, method = "jw") # Compute pairwise Jaro-Winkler distances 
hc <- fastcluster::hclust(as.dist(dist_matrix), method = "average") # Apply hierarchical clustering
threshold <- 0.1 # Define a similarity threshold (lower values mean stricter matching)
clusters <- cutree(hc, h = threshold) # Cut tree into clusters based on threshold

# Assign a representative name for each cluster (first name in each group)
name_mapping <- data.frame(cleaned_owner = unique_owners, cluster = clusters, stringsAsFactors = FALSE) %>%
  group_by(cluster) %>%
  mutate(standardized = first(cleaned_owner)) %>%  # Assign first name in cluster as standard
  ungroup() %>%
...
filtered_parcels_filtered_owners <- filtered_parcels_filtered_owners %>%
  ...
  mutate(final_owner = standardized) %>%
  dplyr::select(-standardized)

st_write(filtered_parcels_filtered_owners%>%filter(parcel_area_ha > area_threshold & fraction_cropland > cropland_threshold), "Data/Parcels/Filtered parcels/filtered_parcels_filtered_owners.gpkg", delete_dsn = T)
filtered_parcels_filtered_owners = st_read("Data/Parcels/Filtered parcels/filtered_parcels_filtered_owners.gpkg")
```
...
agencies_all = st_transform(st_read("Data/Water Agencies/i03_WaterDistricts.shp"), crs)%>%
  ...
  mutate(agency_type = case_when(
    ...
    grepl("Irrigation|Water District|wd|canal|agricultural|improvement district|storage district|slough|water bank|rio oso|San Joaquin River Exchange|bend district", AGENCYNAME, ignore.case = T)   ~ "Irrigation & water districts",
    
    grepl("water agency", AGENCYNAME, ignore.case = T) ~
      "Other types of agencies",
    
    grepl("City|county|school|indian|caltrans|town of|sheriff|state fair|air force|defense|naval|airport", AGENCYNAME, ignore.case = T) ~ "Cities, counties, public land, etc.", 
    
   grepl("school|uc davis|csu|college|research|uc davis|kearney", AGENCYNAME, ignore.case = T) | AGENCYNAME %in% c("Genetic Resource Center") ~  "Education/Research",
...
tm_shape(agencies_all%>%
           filter(grepl("Buttonwillow Community Water District", AGENCYNAME, ignore.case = T) | AGENCYNAME %in% c("")))+
...
agencies_filtered = agencies_all%>%
  filter(agency_type %in% c("Irrigation & water districts", "Individuals and family trusts", "Individual ranches, farms, etc", "Other types of agencies"))
```
...
cropland_parcels_with_agencies = cropland_parcels_filtered_owners%>%
  st_join(., agencies_filtered%>%select(AGENCYNAME, agency_type), join = st_intersects, largest = T)%>%
  mutate(sw_parcel = ifelse(!is.na(AGENCYNAME), 1, 0))

st_write(cropland_parcels_with_agencies, "Data/Parcels/Cropland parcels/cropland_parcels_with_sw_agencies.gpkg")
...
cropland_parcels_with_agencies%>%
  left_join(., cropland_parcels_grouped_overall%>%mutate(cropland_area_percentile = ntile(cropland_area, 100))%>%rename_with(~ str_c("owner_", .), .cols = -cleaned_owner), by = "cleaned_owner")%>%
  group_by(owner_cropland_area_percentile)%>%
  reframe(sw = mean(sw_parcel), sw_area = sum(sw_parcel*cropland_area)/sum(cropland_area))%>%
  ggplot(aes(owner_cropland_area_percentile, sw*100))+
  geom_point()+
  theme_bw()+
...
cropland_parcels_with_agencies%>%
  ...
  left_join(., cropland_parcels_grouped_overall%>%mutate(cropland_area_percentile = ntile(cropland_area, 100))%>%rename_with(~ str_c("owner_", .), .cols = -cleaned_owner), by = "cleaned_owner")%>%
  group_by(cleaned_owner, owner_cropland_area_percentile)%>%
  reframe(sw = mean(sw_parcel), sw_area = sum(sw_parcel*cropland_area)/sum(cropland_area))%>%
  group_by(owner_cropland_area_percentile)%>%
...
parcels_with_agencies = filtered_parcels_filtered_owners%>%
  ...
  st_join(., agencies%>%select(AGENCYNAME), join = st_intersects, largest = T)%>%
  mutate(sw_parcel = ifelse(!is.na(AGENCYNAME), 1, 0))
parcels_with_agencies%>%data.frame()
