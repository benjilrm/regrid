...
# Load packages and set universal variables
...
```{r}
cropland_extent_raster_binary = rast("Crop data/crops croplandcros/cropland_binary.tif")
```
...
gpkg_files = list.files("Regrid parcel data/Raw/", pattern = "\\.gpkg$", full.names = TRUE)
...
```{r}
colnames(st_read("Regrid parcel data/Raw/ca_alpine.gpkg")%>%data.frame())
...
## Processing cropland parcels

Go through the parcels geopackage for each Central Valley county, and
for each parcel, calculate the fraction of the parcel that is cropland
(based on our binary cropland raster above). Label parcels as "cropland
parcels" if at least 10% of the parcel area is designated as cropland.
This is to avoid designating parcels as cropland parcels in cases where
a tiny portion is cropland or a few grid cells are mistakenly classified
as cropland in the Cropland CROS raster. Combine cropland parcels from
all Central Valley counties into one large dataset of cropland parcels.
...
master_cropland_parcels = lapply(central_valley_gpkgs, function(file) {
  ...
  return(st_transform(st_read(file, quiet = TRUE), crs) %>%
...
st_write(master_cropland_parcels, "Regrid parcel data/Processed/central_valley_cropland_parcels_june6.gpkg")
```

# Compiling well and section data
...
wells = read.csv("GW well data/USGWD_California.csv")%>%
  ...
  distinct(Well.ID..State., .keep_all = TRUE)%>%
  rename(ca_well_id = 2)
```
...
```{r}
oswcr = read.csv("GW well data/wellcompletionreports.csv")%>%
...
```{r}
wells_sf = st_transform(st_as_sf(left_join(wells, oswcr, by = "ca_well_id")%>%
...
Import GW basin spatial data (obtained from [CA DWR](https://water.ca.gov/programs/groundwater-management/bulletin-118))
...
gw_basins = st_transform(st_read("Bulletin 118 Groundwater Basin Boundaries/Bulletin 118 Groundwater Basin Boundary GIS Data - v6_2/Shapefile/Shapefile/i08_B118_v6_2.shp"), crs)%>%
  ...
  st_filter(., ca_counties, .pred = st_intersects)
```
...
```{r}
sections = st_transform(st_read("C:/Users/benji/OneDrive/Desktop/Regrid project/PLSS Sections/Public_Land_Survey_System_(PLSS)%3A_Sections.shp"), crs)
```
...
Filter for sections that intersect at least 1 cropland parcel and intersect a GW basin.

```{r}
sections_with_cropland_parcels_and_gw_basins = sections%>%
...
### Wells in relevant sections

Filter for wells that are in sections that intersect with cropland parcels in GW basins. Also for wells with use denoted irrigation or unknown. Process well address to facilitate future matches between wells and parcels based on address.

```{r}
filtered_wells = st_join(wells_sf, sections_with_cropland_parcels_and_gw_basins)%>%
  filter(grepl("Irrigation|Unknown", USGS.Water.Use.Category) & !is.na(MTRS))%>%
...
st_write(filtered_wells, "GW well data/wells_filtered_june6.gpkg")
```

### Relevant sections with wells in them

Before, we filtered for sections intersecting cropland parcels and groundwater basins. Now, we want to narrow down to sections intersecting cropland parcels specifically that have a well in them. This will be our final 'filtered sections' dataset.

```{r}
filtered_sections = sections_with_cropland_parcels_and_gw_basins%>%
  filter(MTRS %in% filtered_wells$MTRS)

rm(sections_with_cropland_parcels_and_gw_basins)
rm(filtered_wells, gw_basins, master_cropland_parcels)

st_write(filtered_sections, "PLSS Sections/filtered_sections_june6.gpkg")
```
...
well_buffer = filtered_wells%>%
  mutate(buffer_distance = as.numeric(gsub("\\s*\\w*$", "", ifelse(geo_accuracy %in% distances_less_than_50_ft, geo_accuracy, NA)))*feet_to_metres)%>%
  filter(!is.na(buffer_distance))%>%
  mutate(geom = st_buffer(geom, dist = buffer_distance))%>%
  dplyr::select(ca_well_id, MTRS, geom)%>%
  rename(MTRS_well = MTRS)
```
...
## Iterate through counties and parcels
...
st_write(filtered_parcels, "Regrid parcel data/Processed/central_valley_filtered_parcels_june6.gpkg")
```

## Join parcels with section

Create a unique ID (my_parcel_num) for each parcel, since the parcel IDs can be duplicated in some cases. Spatially join these filtered parcels with the section they intersect. If a parcel intersects 2 sections, this will create a duplicate row for that parcel (ie: the parcel will have two rows, one for each section it intersects). We want this because we want to consider all possible sections a parcel could intersect rather than just the one it has most overlap with.

```{r}
filtered_parcels = st_read("Regrid parcel data/Processed/central_valley_filtered_parcels_june6.gpkg")%>%
  mutate(my_parcel_num = row_number())

filtered_parcels_with_section = st_join(filtered_parcels, filtered_sections)

st_write(filtered_parcels_with_section, "Regrid parcel data/Processed/filtered_parcels_with_section_june11.gpkg")
```

## Well buffer

Using spatial join, create a dataset of parcels that intersect with the designated buffer of any of the wells that had a geolocation geoaccuracy 50ft or less (which are make up the 'well_buffer' dataset). In cases where there are multiple rows for a given parcel, only keep the row where the section code (MTRS) matches that of the well whose buffer the parcel intersects.

```{r}
well_buffer_parcels = st_filter(filtered_parcels_with_section, well_buffer%>%select(ca_well_id, MTRS_well), .pred = st_intersects)%>%
  st_join(., well_buffer)%>%
  data.frame()%>%
  filter(MTRS == MTRS_well)
st_write(well_buffer_parcels, "Regrid parcel data/Processed/well_buffer_parcels_june11.gpkg")
```
...
Clean up the IDs, addresses, and such for wells and land parcels (ex: turn addresses to lowercase so capitalisation won't trip up any of the well-parcel matching)

```{r}
filtered_parcels_with_section = st_read("Regrid parcel data/Processed/filtered_parcels_with_section_june11.gpkg")%>%
  data.frame()%>%
...
filtered_wells = st_read("GW well data/wells_filtered_june6.gpkg")%>%
  data.frame()%>%
  mutate(APN = gsub("[- ]", "", APN),
         well_address = str_to_lower(well_address),
         address_st = str_to_lower(address_st))

well_buffer_parcels = st_read("Regrid parcel data/Processed/well_buffer_parcels_june11.gpkg")
well_buffer_parcels_ids = unique(well_buffer_parcels$ca_well_id)
```

## Run matching loop

Create an empty dataframe for each well and the section it falls in that
we will populate with the parcel number it gets matched with (if any)
and the matching method.

```{r}
matches = data.frame(my_parcel_num = NA, ca_well_id = filtered_wells$ca_well_id, string_dist = NA, match_method = NA, section = filtered_wells$MTRS)%>%
...
Then, for each well, we go through our hierarchy for matching methods. First priority is if the well's section or buffer only has one parcel in it (which makes it easy for us!). Second is if the APN (Assessor Parcel Numbers) of the well and parcel address perfectly match. Third is if well address is a perfect match with a parcel in that section's address or mailing address. Fourth is if the number in the well and parcel address number is the exact same and the street name is very similar (less than 0.3 based on Jaro-Winkler string distance)

```{r}
for (i in 20739:nrow(filtered_wells)) {
  ...
  if (nrow(intersecting_parcels_route1) == 1) {
    matches$my_parcel_num[i] = intersecting_parcels_route1$my_parcel_num
    matches$match_method[i] = if (well$ca_well_id %in% well_buffer_parcels_ids) "1 parcel in well buffer" else "1 parcel in section"
    next} 
  ...
  apn_match[is.na(apn_match)] = FALSE
    if (any(apn_match, na.rm = T)) {
      matches$my_parcel_num[i] = intersecting_parcels$my_parcel_num[apn_match][1]
      matches$match_method[i] = "APN match"
  ...
  if (any(address_match, na.rm = T)) {
    matches$my_parcel_num[i] <- intersecting_parcels$my_parcel_num[address_match][1]
    matches$match_method[i] <- "Perfect address match"
  ...
  if (!is.na(well$address_no) & !is.na(well$address_st)){
    ...
    if (any(address_number_match, na.rm = T)) {
      ...
      if (distances[min_dist] < 0.3) {
        matches$my_parcel_num[i] <- intersecting_parcels$my_parcel_num[address_number_match][min_dist]
        matches$match_method[i] <- "String distance match"
        matches$string_dist[i] <- distances[min_dist]
        next}}}}
```

Export our completed match dataset
```{r}
write.csv(matches, "Matches/matches_full.csv", row.names = F)
```

Diagnostics

```{r}
table(matches%>%filter(!is.na(my_parcel_num))%>%
...
matches%>%filter(!is.na(my_parcel_num))%>%
  left_join(., filtered_wells%>%select(ca_well_id, APN), by = "ca_well_id")%>%
  left_join(., filtered_parcels_with_section%>%select(my_parcel_num, parcel_id, parcelnumb_no_formatting, MTRS), by = c("my_parcel_num" = "my_parcel_num", "section" = "MTRS"))
```
...
Prepare and export unmatched wells to geocode as excel file to read into ArcGIS for geocoding. Filter out any wells whose addresses are ungeocodable (ex: any part of the address is NA)

```{r}
matches = read.csv("Matches/matches_full.csv")

matches

wells_to_geocode = filtered_wells%>%
  filter(ca_well_id %in% (matches%>%filter(is.na(my_parcel_num))%>%pull(ca_well_id)) & !is.na(well_address) & address_no != 0 & grepl("^[0-9]+$", address_no) & !is.na(city))%>%
  mutate(state = "CA")

write.xlsx(wells_to_geocode%>%select(ca_well_id, well_address, city, state), 'wells_to_geocode.xlsx', row.names = FALSE)
```
...
to an actual address (rather than an intersection, POI, etc., which would mess up our spatial join with parcels). Spatially join geocoded wells to parcels.

```{r}
#pull geocoded wells back in from arcgis geocoding service and filter for actual addresses
geocoded_wells = st_transform(st_read("geocoded_wells.shp"), crs = crs)%>%
  filter(!is.na(Addr_type) & !Addr_type %in% c("StreetName", "StreetMidBlock", "Locality", "StreetInt", "POI") & Score > 90)%>%
  select(USER_ca_we)%>%
  rename(ca_well_id = 1)

#spatial join geocoded points to parcels
...
write.csv(final_matches, "Matches/final_matches_with_geocoding_august2.csv", row.names = F)
```
